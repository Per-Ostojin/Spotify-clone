...
const PlayerControls = ({ is_paused, duration, progress, player }) => {
    ...
    return ( <Stack direction={'column'} spacing={2} justifyContent={'center'} alignItems={'center'} sx={{ width: '100%' }} >
        <Stack spacing={1} direction={'row'} justifyContent={'center'} alignItems={'center'} sx={{ width: '100%' }} >
            <IconButton size='small' sx={{color: 'text.primary'}} >
                <SkipPrevious sx={skipStyle} />
            </IconButton>
            <IconButton size='small' sx={{color: 'text.primary'}} >{is_paused ? <PlayArrow sx={playStyle} /> : <Pause sx={{playStyle}} /> }</IconButton>
            <IconButton size='small' sx={{color: 'text.primary'}} >
